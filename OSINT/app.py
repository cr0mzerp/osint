...
# --- ALIST PROXY ---
@app.route('/alist/<path:path>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def alist_proxy(path):
    ...
    try:
        if request.method == 'GET':
            resp = requests.get(alist_url, timeout=30)
        elif request.method == 'POST':
            resp = requests.post(alist_url, json=request.get_json(), headers=dict(request.headers), timeout=30)
        elif request.method == 'PUT':
            resp = requests.put(alist_url, json=request.get_json(), headers=dict(request.headers), timeout=30)
        elif request.method == 'DELETE':
            resp = requests.delete(alist_url, timeout=30)
        else:
...
def alist_root():
    """Redirect to Alist root."""
    return redirect(url_for('alist_proxy', path=''))
