...
def proxy_request(alist_url, method):
    ...
    try:
        if method == 'GET':
            ...
            resp = requests.post(alist_url, headers=headers, data=data if method in ['POST', 'PUT', 'PATCH'] else None, timeout=30, allow_redirects=True)
        elif method == 'PUT':
            resp = requests.put(alist_url, headers=headers, data=data if method in ['POST', 'PUT', 'PATCH'] else None, timeout=30, allow_redirects=True)
        elif method == 'DELETE':
            resp = requests.delete(alist_url, headers=headers, timeout=30, allow_redirects=True)
        elif method == 'PATCH':
            resp = requests.patch(alist_url, headers=headers, data=data if method in ['POST', 'PUT', 'PATCH'] else None, timeout=30, allow_redirects=True)
        else:
