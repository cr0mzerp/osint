...
# Set OSINT_LOG_ROOT to the mounted Terabox directory
export OSINT_LOG_ROOT="/terabox_data"
...
# Mount point oluştur
mkdir -p /terabox_data

# Rclone mount (background)
rclone mount alist-webdav:/terabox /terabox_data \
    --vfs-cache-mode full \
...
sleep 2
if mountpoint -q /terabox_data; then
    echo "Mount başarılı: /terabox_data"
    ls -la /terabox_data | head -20
else
